...
Стоит отметить особенность при обходе векторов с помощью `map` и `filter`, что при использовании обычных вариантов этих функций, результат будет упакован в список, если же необходимо получить результат в виде вектора, для функций `map` и `filter` используются их аналоги `mapv` и `filterv`. Примеры:
