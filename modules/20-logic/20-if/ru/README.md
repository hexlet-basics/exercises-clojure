> (if test-expr then-expr else-expr) Условная конструкция в Clojure – это специальная форма, в которой 4 элемента. Первый – `if`, затем выражение-предикат (`test-expr`). Если предикат вернул истину, то выполняется `then-expr`, иначе `else-expr`. В Clojure `if` всегда содержит ветку `else`.
