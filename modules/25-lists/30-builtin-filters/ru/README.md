...
Результатом вызова `filter` будет _новый_ список, содержащий элементы, одобренные аргументом-предикатом. Благодаря тому, что в Clojure доступно великое множество функций-предикатов, использовать `filter` легко и удобно:
